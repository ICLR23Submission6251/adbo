...
def save(exp_time, eph, csvlog: pd.DataFrame):
    ...
    if not os.path.exists(logfolder):
        ...
        with open(logfolder + 'setting.txt', 'w') as fmeta:
            EXP_ENV_KEYS = [
                'seed', 
                'noise_rate', 'w_lr', 'hp_lr', 'batch_size',
            ...
            fmeta.write(f'exp: mnist_hp_adbo\n')
...
def main():
    ...
    t_imgs = test_data.data.clone().detach()
    t_imgs = t_imgs.to(device=device, dtype=torch.float32)
    t_imgs = t_imgs.reshape([t_imgs.shape[0], -1])
    t_imgs = ((t_imgs / 255.0) - 0.1307) / 0.3081
    t_lbls = test_data.targets.clone().detach()
    t_lbls = t_lbls.to(device=device)
    ...
    torch.manual_seed(seed)
    torch.nn.init.kaiming_normal_(z, mode='fan_out')
    ws = dict(zip(NODES, [z.clone().detach() for _ in range(NODES_CNT)]))
    v = torch.zeros((60000,), device=device)
    ...
    for i, nd in enumerate(NODES):
        payload = {'sender': 'main', 'seed': seed+(i+1)}
        channel.basic_publish('', nd, body=json.dumps(payload), properties=propWithModeHeader('init_seed'))
    ...
    for k in range(epochs):
        ...
        if k % pre_epochs == 0:
            ...
            for _ in range(phix_iter):
                ...
                gradz = gradz + mu * sum([(h2z - h2ynd) for h2ynd in h2y.values()])
                h2z = h2z - z_lr * gradz
                # send back
                for nd in active_nds:
                    channel.basic_publish('', nd, body=pack_tensors('main', phi=phi[nd], zk=h2z), properties=propWithModeHeader('step_newcp'))
