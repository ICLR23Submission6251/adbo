...
def nositify(lbls):
    NOISE_RATE = float(os.environ['noise_rate'])
    num = int(NOISE_RATE*(lbls.shape[0]))
    randint = torch.randint(1, 10, (num,), device=device)
    index = torch.randperm(lbls.shape[0], device=device)[:num]
    labels = lbls.clone()
    labels[index] = (labels[index]+randint) % 10
    return labels


def loss_g(imgs, lbls, w, hp_slice):
    o = torch.matmul(imgs, w)
    l = functional.cross_entropy(o, lbls, reduction='none')
    l = torch.mul(l, torch.sigmoid(hp_slice)).mean() + 0.001 * torch.pow(torch.norm(w), 2)
    return l


def prep_data(sample_start, sample_end, batch_size, val_ratio):
    train_data = torchvision.datasets.MNIST(root='./data/', train=True, download=True)
    train_imgs = train_data.data[sample_start:sample_end].clone().detach()
    train_imgs = train_imgs.to(device=device, dtype=torch.float32)
    train_imgs = train_imgs.reshape([sample_end-sample_start, -1])
    train_imgs = ((train_imgs / 256.0) - 0.1307) / 0.3081
    train_lbls = train_data.targets[sample_start:sample_end].clone().detach()
    train_lbls = train_lbls.to(device=device)
    ...
    for imgs, lbls in train_set:
        ...
        train_lbls_list.append(nositify(lbls))
...
def main():
    ...
    hparam = torch.zeros((60000,), device=device, requires_grad=True)
    b = None
    wk_approx = None
    v = None
    l = None
    ...
    def step_newcp(phi, zk):
        try:
            ...
            twk = wk_approx.clone().detach().requires_grad_()
            idx = np.random.randint(train_batch_num)
            hp_slice = v[sample_start+idx*batch_size:sample_start+(idx+1)*batch_size]
            l = loss_g(train_imgs_list[idx], train_lbls_list[idx], twk, hp_slice)
            if phi is not None:
                l = l + torch.mul(phi, twk).sum()
            l = l + 0.5 * mu * torch.pow(twk - zk, 2).sum()
            grad_wk = torch.autograd.grad(l, twk, create_graph=True)[0]
    ...
    def cb_recv(ch, method, props, body):
        try:
            ...
            elif msg_mode == 'init_seed':
                nonlocal seed
                seed = json.loads(body)['seed']
                torch.manual_seed(seed)
                np.random.seed(seed)
                torch.nn.init.kaiming_normal_(param, mode='fan_out')
